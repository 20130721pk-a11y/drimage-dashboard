...
const PLATFORM_COLORS: Record<string, string> = {
  '유튜브': '#ef4444', '치지직': '#3b82f6', 'SOOP': '#f59e0b'
}
...
const SOURCE_MAP: Record<string, string> = {
  '구글 뉴스': 'Google News', '네이버 뉴스': '네이버 -', '네이버 블로그': '네이버블로그',
}
...
export default function Home() {
  const [activeTab, setActiveTab] = useState<'news' | 'streams'>('news')
  ...
  async function fetchAll() {
    setLoading(true)
    const [{ data: newsData }, { data: streamData }] = await Promise.all([
  ...
  const categoryCount = ['자사', '경쟁사', '업계'].map(cat => ({
    ...
    segments: SEGMENTS[cat].map(seg => ({
      name: seg, value: news.filter(n => n.category === cat && (n.tags?.includes(seg) || n.title?.includes(seg))).length
    }))
  }))
