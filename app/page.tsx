...
export default function Home() {
  ...
  async function fetchAll() {
    ...
    const [{ data: n }, { data: ca }, { data: s }, { data: p }] = await Promise.all([
      ...
      supabase.from('competitor_ads').select('*').order('published_at', { ascending: false }).limit(2000),
      supabase.from('streams').select('*').gte('collected_at', new Date(Date.now()-90*24*60*60*1000).toISOString()).order('collected_at', { ascending: false }).limit(5000),
  ...
  const filteredNews = useMemo(() => news.filter(n => {
    ...
    const isWebzine = n.source?.includes('인벤') || n.source?.includes('루리웹') || n.source?.includes('thisisgame') || n.source?.includes('미니맵')
...
<div className="col-span-3 bg-gray-800 rounded-2xl p-5 border border-gray-700">
              ...
              {activeTab === 'ads' && (
                <div>
                        ...
                        <div className="bg-gray-800 rounded-2xl p-4 border border-gray-700">
                          ...
                          <ResponsiveContainer width="100%" height={180}>
                            <BarChart data={byCompetitor} barCategoryGap="20%">
                              ...
                              <Bar dataKey="count" radius={[4,4,0,0]} onClick={(data:any)=>setAdCompetitor(data.fullName===adCompetitor?'전체':data.fullName)} style={{cursor:'pointer'}}>{byCompetitor.map((entry,i)=><Cell key={i} fill={COMP_COLOR[entry.fullName]||'#6366f1'} opacity={adCompetitor==='전체'||adCompetitor===entry.fullName?1:0.3}/>)}</Bar>
                            </BarChart>
                          </ResponsiveContainer>
                        </div>
                        <div className="bg-gray-800 rounded-2xl p-4 border border-gray-700">
                          ...
                          <ResponsiveContainer width="100%" height={180}>
                            <PieChart>
                              <Pie data={byType} cx="50%" cy="50%" innerRadius={45} outerRadius={70} dataKey="value" label={({name,percent}:any)=>`${name} ${((percent||0)*100).toFixed(0)}%`} labelLine={false} fontSize={10} onClick={(data:any)=>setAdType(data.rawType===adType?'전체':data.rawType)} style={{cursor:'pointer'}}>
                                {byType.map((entry:any,i:number)=><Cell key={i} fill={PIE_COLORS[i%PIE_COLORS.length]} opacity={adType==='전체'||adType===entry.rawType?1:0.3}/>)}
                              </Pie>
                              <Tooltip contentStyle={{background:'#1f2937',border:'1px solid #374151',borderRadius:'8px',fontSize:'12px'}}/>
                            </PieChart>
                          </ResponsiveContainer>
                  ...
                  {adTab==='크리에이티브' && (()=>{
                    const filtered=competitorAds.filter(a=>(adPlatform==='전체'||a.platform===adPlatform)&&(adCompetitor==='전체'||a.competitor.includes(adCompetitor))&&(adRegion==='전체'||a.region===adRegion)&&(adType==='전체'||a.ad_type===adType))
                    ...
                    return <div className="grid grid-cols-4 gap-4">{filtered.slice(0,40).map(ad=>{
                      ...
                      const typeLabel = ad.ad_type==='Video'?'동영상':ad.ad_type==='Image'?'이미지':'텍스트';
                      return (<a key={ad.id} href={ad.url} target="_blank" rel="noopener noreferrer" className="bg-gray-800 rounded-2xl border border-gray-700 hover:border-gray-500 transition-all overflow-hidden group">
                        <div className="w-full h-40 bg-gray-750 overflow-hidden relative flex items-center justify-center" style={{background:'#1a1f2e'}}>
                          {ad.thumbnail
                            ? <img src={ad.thumbnail} alt="" className="w-full h-full object-cover"/>
                            : <div className="flex flex-col items-center gap-2 text-gray-500">
                                <span className="text-3xl">{typeIcon}</span>
                                <span className="text-xs">{typeLabel} 광고</span>
                                <span className="text-xs text-gray-600">클릭하여 보기</span>
                              </div>
                          }
